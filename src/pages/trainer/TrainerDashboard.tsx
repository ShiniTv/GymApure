...
function TodayPanel({
  ...
  return (
    ...
    >
      <h2 className="mb-2.5 text-[13px] font-semibold text-zinc-900 lg:mb-3 lg:text-sm dark:text-white">
        Hoy
      </h2>
      {loading ? (
                ...
                {trainingToday.slice(0, TODAY_LIST_CAP).map((m) => (
                  <li key={m.id}>
                    <Link
                      ...
                      className="hover:text-brand flex items-center justify-between gap-2 rounded-lg py-1.5 text-[12px] font-medium text-zinc-800 lg:py-2 lg:text-[13px] dark:text-zinc-200"
                    >
...
export default function TrainerDashboard() {
  ...
  return (
    <div className="mx-auto w-full max-w-3xl space-y-4 sm:space-y-5 lg:max-w-5xl lg:space-y-6">
      ...
      {/* Mobile: stack · Desktop: Hoy + Actividad side by side */}
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1.15fr)_minmax(0,0.85fr)] lg:items-stretch lg:gap-5">
        <TodayPanel
          loading={loading}
          quiet={todayQuiet}
