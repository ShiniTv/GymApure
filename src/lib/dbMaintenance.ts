...
export async function runDbMaintenance(): Promise<DbMaintenanceResult> {
  ...
  const readNotifRetentionDays = parseRetentionDays(
  ...
  const [audit, notifLog, readNotifications, resetTokens, pushSubscriptions] = await Promise.all([
    ...
    query(
      `DELETE FROM push_subscriptions ps
       WHERE ps.updated_at < NOW() - ($1::text || ' days')::interval
          OR NOT EXISTS (
            SELECT 1 FROM users u WHERE u.id = ps.user_id AND u.status = 'active'
          )`,
