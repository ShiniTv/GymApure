...
const PUBLIC_PROBES: EndpointProbe[] = [
  { name: 'health', path: '/api/health', maxMs: 500 },
];

const AUTH_PROBES: EndpointProbe[] = [
...
console.log('  ✓ stats admin/member — Promise.all en oleadas');
...
const demoEmail = process.env.AUDIT_DEMO_EMAIL ?? 'admin@gymapure.demo';
const demoPassword = process.env.AUDIT_DEMO_PASSWORD ?? process.env.DEMO_PASSWORD ?? 'DemoAdmin123!';
const token = await loginAs(demoEmail, demoPassword);

if (!token) {
  console.warn('\n⚠ No se pudo autenticar — omitiendo probes autenticados');
  console.warn(`  Usa AUDIT_DEMO_EMAIL / AUDIT_DEMO_PASSWORD o DEMO_PASSWORD`);
} else {
