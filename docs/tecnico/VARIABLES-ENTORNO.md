...
| `REQUIRE_MFA_FOR_STAFF` | `true` en prod: staff debe activar MFA en `/security` antes de la API   |
...
| `ADMIN_NOTIFY_EMAILS` | CSV de correos del gym (alerta cuando hay pago pendiente de revisión)                            |
| `PUBLIC_APP_URL`      | URL HTTPS pública del sitio (obligatorio en prod; enlaces de reset, logo y CTAs)                 |
| `DATABASE_SSL_CA`     | Ruta al CA **o PEM inline** (`-----BEGIN CERTIFICATE-----…`) para verify-full (recomendado prod) |
| `DEV_LOG_RESET_LINKS` | `true` imprime enlaces de reset en consola (solo desarrollo)                                     |

Sin SMTP: no hay correos de bienvenida, recuperar contraseña, pagos ni vencimiento. Con SMTP, los avisos de vencimiento van por **correo + chat in-app**. `ADMIN_NOTIFY_EMAILS` es independiente del SMTP del cliente: usa el mismo transport.
