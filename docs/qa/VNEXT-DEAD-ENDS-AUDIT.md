...
| `/access-denied`          | todos  | Sin `state.from`; hints muertos                      | P1        | Hecho (`state.from` + CTAs por rol; coaching vs ops) |
...
2. Cobros PT condicional + empties accionables — hecho; AccessDenied `from` hecho
